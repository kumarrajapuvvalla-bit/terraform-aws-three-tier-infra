...
provider "aws" {
  ...
  default_tags {
    tags = { Project = var.project_name, Environment = var.environment, ManagedBy = "Terraform", Owner = var.owner }
  }
}
...
locals {
  ...
  common_tags = { Project = var.project_name, Environment = var.environment, ManagedBy = "Terraform", Owner = var.owner, Workspace = terraform.workspace }
}
